...
public class PopulateNextPointersOfTree {
    public Node connect(Node root) {

        if (root == null)
            return null;
        //if root.left not null it is always connected to right
        if (root.left != null)
            root.left.next = root.right;
        if (root.right != null){
            //if root.right not null then 2 options.
            //if root.next==null then even root.right.next is null.
            //else roo.right.next is root.next.left.
            //Dont forget its complete Binary tree
            if(root.next==null){
