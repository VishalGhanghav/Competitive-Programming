...
public class LongestBitonicSequenceStrict {
    public static int LongestBitonicSequence(int n, int[] arr) {
        ...
        int lbsLength = 0;
        for (int ind = 0; ind < n; ind++) {
            // If both increasing and decreasing parts are non-empty
            if (dp1[ind] > 1 && dp2[ind] > 1) {
                int length = dp1[ind] + dp2[ind] - 1; // Subtract 1 to avoid counting the peak twice
                lbsLength = Math.max(lbsLength, length);
            }
        }
        return lbsLength;
    }
    ...
    public static void main(String[] args) {
        // Test cases for the strictly increasing or decreasing not valid
        int[] arr1 = {1, 2, 5, 3, 2};
        ...
        int[] arr3 = {5, 6, 7};
        System.out.println("Longest Bitonic Subsequence Length: " + LongestBitonicSequence(arr3.length, arr3)); // Expected output: 0 (since it's only increasing)
    }
}
